...
여기에 문의 채널을 하나 더 붙입니다. 서버가 아직 안 만들어 순서는 뒤입니다. 계약은 기획이 먼저
정해 넘겼고 화면은 5장에 따로 적었습니다.
...
사이드바 항목은 역할과 무관하게 고정입니다. 처음에는 셋이고 문의 채널이 붙으면 넷이 됩니다.
달라지는 건 각 항목 안쪽입니다.
...
ADR-011은 Status가 `Proposed`인 데다 몇 군데를 기획 확정 대기로 비워 뒀습니다. 그 빈칸은
`docs/server/문의_문의 채널_서버명세.md`에서 채워 서버팀에 넘겼습니다. 아래는 그 명세 기준이고
ADR 원안과 달라진 곳은 명세 5장에 표로 모아 뒀습니다.

REST 1차 범위:

| Method | Path | 쓰임 |
|---|---|---|
| GET | `/api/v1/inquiry-categories` | 카테고리 목록. 등록 폼을 이걸로 그림 |
| POST | `/api/v1/inquiries` | 문의 등록. `{ categoryId, content, fileIds[] }` |
| GET | `/api/v1/inquiries` | 목록. `status`·`categoryId`·`unreadOnly` 필터에 커서 페이징 |
...
카테고리는 서버가 enum으로 박아 두지 않습니다. 프로덕트팀이 관리하는 테이블이라 목록이 운영
중에 바뀝니다. 앱은 하드코딩하지 않고 열 때마다 받아 씁니다.

대상도 앱이 고르지 않습니다. 카테고리마다 `target`이 붙어 있고 서버가 그걸로 라우팅합니다.
등록 요청에 `InquiryTarget`을 넣지 않고 카테고리 조회 응답에도 `target`은 오지 않습니다.
챌린저가 자기 질문의 소관을 알아야 쓸 수 있는 폼이 되는 걸 막으려는 겁니다.

제목도 없습니다. 등록 폼은 카테고리 하나와 본문, 첨부뿐입니다. 목록에 뜨는 제목은 서버가
본문 앞 40자를 잘라 만듭니다.

역할은 둘입니다. 문의를 연 사람이 `INQUIRER`, 대상 운영진이 `RESPONDER`입니다. 본인이 쓴 문의를
본인이 처리할 권한까지 가진 경우에는 `INQUIRER`가 이깁니다. 대상은
`CENTRAL`·`CHAPTER`·`SCHOOL`·`UMC_PRODUCT` 넷이고 `UMC_PRODUCT`는 프로덕트팀이 받습니다.
다만 서버에 프로덕트팀을 판별할 수단이 아직 없어서 그게 서기 전에는 앱 문의 카테고리를
만들 수 없습니다. 운영 문의 세 대상은 그 전에도 열립니다.
...
- **미읽음은 운영진 쪽만 1차 범위입니다.** 문의자 화면에는 안 읽음 배지가 없습니다.
  명세에 서버갭으로 올려 뒀지만 우선순위는 뒤입니다

### 맥에서의 모양

3컬럼에 그대로 들어갑니다. 사이드바 네 번째 항목이고 목록은 필터가 붙은 문의 목록,
상세는 메시지 타임라인과 입력창입니다.

역할에 따라 목록의 성격이 갈립니다. 챌린저에게는 본인 문의만 오니 대개 한두 건이고
비어 있으면 문의하기 버튼만 남습니다. 운영진에게는 권한 범위의 문의가 전부 쌓이니
상태와 카테고리로 거르는 `Table`이 필요합니다. 카테고리 필터 항목은 서버에서 받은 목록으로
그립니다. 운영 중에 늘거나 줄 수 있어서 앱에 박아 두면 어긋납니다. 스레드보다 공지 읽음
통계에 가까운 화면입니다.

실시간은 8장에서 정한 것을 그대로 씁니다. STOMP 커넥션은 앱에 하나고 필터 키가
`threadId` 하나에서 `inquiryId`까지 늘어날 뿐입니다. 잠자기에서 깨어난 뒤
`GET /api/v1/inquiries/{id}/messages`로 채워 넣는 것도 스레드와 같습니다.

낙관적 업데이트는 메시지 전송에만 씁니다. 실패해도 다시 보내면 그만입니다. 종료는
8장의 심사 판정과 같은 이유로 서버 응답을 보고 반영합니다.

### 언제 붙이나

로드맵 6단계입니다. 서버 진행에 따라 두 토막으로 나눌 수 있습니다. 카테고리 API와 메시지
REST까지 끝나면 실시간 없이도 화면을 열 수 있습니다. 열 때와 ⌘R에 새로고침하는 정도면 됩니다.
문의는 스레드만큼 즉시성이 필요한 대화가 아닙니다. WebSocket이 끝나면 구독을 붙입니다.

지금 당장 할 일은 클라이언트 쪽에 있습니다. 2단계에서 스레드 실시간을 만들 때 `threadId`에
묶인 코드를 쓰지 않는 것입니다. 목적지 문자열과 필터 키만 갈아 끼우면 문의가 얹히게 해 두면,
서버가 준비됐을 때 화면만 그리면 됩니다.

## 6. 공유 패키지 — UMCNetworkKit
...
- **서버 문의 도메인 착수 일정.** ADR-011이 언제 구현으로 넘어가느냐에 6단계가 통째로 걸려
  있습니다. 기획 쪽 빈칸은 서버명세로 넘겼으니 남은 건 서버 일정과, 프로덕트팀을 판별할 수단을
  언제 만드느냐입니다
