...
| Method | Path | 쓰임 |
|---|---|---|
| POST | `/api/v1/inquiries` | 문의 등록, 첨부는 `fileId[]`로 넘김 |
| GET | `/api/v1/inquiries` | 목록. `status`·`target`·`category`·`unreadOnly` 필터 |
...
`CENTRAL`·`CHAPTER`·`SCHOOL`·`UMC_PRODUCT` 넷인데 `UMC_PRODUCT`는 서버도 아직
중앙운영사무국 전체로 임시 매핑해 뒀습니다.
...
- **서버 문의 도메인 착수 일정.** ADR-011이 언제 구현으로 넘어가느냐에 6단계가 통째로 걸려
  있습니다. `InquiryCategory` 목록과 `UMC_PRODUCT` 대상 매핑도 서버가 확정해야 합니다
